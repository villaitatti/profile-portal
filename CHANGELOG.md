...
- **Email delivery survives a busy mail service.** A temporary rejection from the mail provider marked the message permanently failed, so a short burst of throttling left messages the daily send would never retry. Temporary failures are now retried automatically and only genuine rejections are treated as final. Outgoing mail also now carries a reply address, so a Fellow replying to an invitation reaches somebody.
