...
- **Manage Appointees** — Staff dashboard with a seven-state lifecycle column (*Nominated* → *Nomination Sent* → *Form Submitted* → *Accepted* → *VIT ID Sent* → *VIT ID Claimed* → *Enrolled*) derived from `(fellowshipAccepted, VIT ID match tier, invitation event, bio email event, form invitation events)`. Three manual actions per row — **Generate Form Link** (creates a token-based form invitation scoped to the current fellowship), **Send VIT ID email** (invites a new appointee to claim), and **Send bio email** (requests bio + project description from a claimed appointee) — the email actions route through a shared email preview modal so Angela sees the full rendered HTML before hitting Send.
